...
# for running Power System Simulations.
...
# `PowerSystems` is used to properly define the data structure, while `Sundials` is
...
# In here, he will use a Three Phase Fault, that is modeled by modifying the
# admittance matrix of the system. To do so we create a NetworkSwitch perturbation
# as follows:
...
# After running the simulation, our simulation structure `sim` will have the solution. For
# that `sim.solution` can be used to explore the solution structure. In this case
# `sim.solution.t` returns the vector of time, while `sim.solution.u` return the array of
# states. In addition, `PowerSimulationsDynamics` have two functions to obtain different
